...
Before we start writing new tests, let's establish a baseline by running the existing test suite and checking our current code coverage. We can use the what we learned in Exercise 2 about how to run tests in this project.
...
3. **Important**: Remember that while GitHub Copilot is very powerful it is still a best practice to always to review and validate the code generated by AI tools. Make sure to:
